...
export async function readJournalData() {
    ...
    return {
        ...
        snapshot: {
            ...
            completedTasks: snapshot['Completed tasks']
                ? snapshot['Completed tasks'].split(',').map((item) => item.trim()).filter(Boolean)
                : [],
            failedTasks: snapshot['Failed tasks']
                ? snapshot['Failed tasks'].split(',').map((item) => item.trim()).filter(Boolean)
                : [],
            completedIssues: snapshot['Completed issues']
                ? snapshot['Completed issues'].split(',').map((item) => item.trim()).filter(Boolean)
                : [],
            failedIssues: snapshot['Failed issues']
                ? snapshot['Failed issues'].split(',').map((item) => item.trim()).filter(Boolean)
                : [],
